...
function updateActionUrls (body) {
  for (let possibleTransition of body.actions) {
    ...
    if (action) {
      ...
      if (possibleTransition.fields) {
        action.template = possibleTransition.fields
      }
    }
  }
...
// Checks if all fields from the template are filled by the input
function checkTemplateCompliance (input, template) {
  let result = true
  for (let field of template) {
    if (typeof input[field.name] === 'undefined') {
      result = false
    } else if (field.type instanceof Array) {
      result = result && checkTemplateCompliance(input[field.name], field.type)
    }
  }
  return result
}
...
exports.updateApplication = function (application) {
  ...
  if (!checkTemplateCompliance(application, action.template)) return false
  return client.requestAPI(action, application)
    .then((body) => {
      updateActionUrls(body)
      return true
